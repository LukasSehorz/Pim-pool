...
function benefitsFor(slug: string): ProductContent["benefits"] {
  ...
  return baseBenefits.map((benefit) => ({
    ...benefit,
    text: `${benefit.text} Speziell ausgelegt für ${label} und Ihre Poolsituation in Oberbayern.`,
  }));
}
...
function getEnclosureDetail(slug: string) {
  const details: Record<string, { subtitle: string; description: string[]; faq: ProductContent["faq"] }> = {
    ...
    practic: { subtitle: "Die funktionale Lösung für zuverlässigen Schutz ohne unnötige Extras.", description: ["Practic konzentriert sich auf das Wesentliche: stabile Konstruktion, einfache Bedienung, guter Schmutzschutz und attraktive Wirtschaftlichkeit.", "Sie eignet sich besonders für Bestandsbecken, Ferienhäuser und Poolbesitzer, die eine robuste Überdachung mit fairem Preis-Leistungs-Verhältnis suchen.", "pim POOL achtet trotzdem auf saubere Schienenführung, passende Dimensionierung und langlebige Materialien."], faq: [{ q: "Ist Practic eine Einstiegslösung?", a: "Ja – aber keine Billiglösung. Sie ist robust und bewusst funktional gehalten." }] },
...
export const CONTENT: Record<string, ProductContent> = Object.fromEntries([
  ...
  make(
    ...
    "Was ist Ionisierung? Wie sicher ist sie? Wie wirkt sie? Antworten von Ihrem pim POOL-Team.",
    "FAQ",
    [
  ...
  make(
    ...
    [
      ...
      "pim POOL plant das System mit Ihnen architektonisch sauber: Belag, Laufrichtung, Bedienung, Entwässerung und Sicherheitsdetails werden exakt auf Garten, Pool und Alltag abgestimmt.",
    ],
